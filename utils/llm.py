...
class OpenAIClient(Client):
  ...
  def create(self, model, messages, reasoning=None, _recur=0):
...
class Agent:
  def __init__(self, model: Model, system_prompt_file: Path, system_prompt_vars={}, multiturn=True):
    self.model = model
  ...
  def run(self, prompt, model=None, reasoning=Ellipsis):
    ...
    response = self.client.create(model or self.model.model, self.messages, reasoning_effort)
    if self.multiturn:
